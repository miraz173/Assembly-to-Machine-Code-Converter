...
function App() {
  ...
  useEffect(() => {
    try {
      ...
      // console.log(operand2, i, insts[i]);
      if (insts[i][0] === "00") {
  ...
  function goForward() {
    console.log(pa, pf, pi, i, xcode.length);
    // let x = [cf, sf, of, zf];
    // setPf((prev) => [...prev, [...x]]);
    if (i < xcode.length - 1) {
      setPa((prev) => [...prev, [...a]]);
...
// function App() {
//   const [dir, setDir] = useState(1);
//   const [pa, setPa] = useState(Array(8).fill(0));
//   const [pf, setPf] = useState(Array(4).fill(0));
//   const [pi, setPi] = useState(0);
...
//   const [zf, setZf] = useState(0);
//   const [of, setOf] = useState(0);
//   const [a, setA] = useState(Array(8).fill(0));
...
//             while (z.length < 7) {
//               z = "0" + z;
//             } //padStart(7, "0") does the same work.
//             setMachineCode((prev) => prev + z + "\n");
//           } else setMachineCode((prev) => prev + "undefined\n");
//         } else if (x.length === 3) {
//           // Arithmetic instructions: ADD R1, R2
//           setMachineCode((prev) => {
//             return prev + (isNaN(x[2]) ? "00 " : "01 ");
...
//             while (z.length < 3) {
//               z = "0" + z;
//             } //padStart(3, "0") does the ame work.
//             setMachineCode((prev) => prev + z + " 0\n");
//           } else {
//             //4
//             let z = parseInt(x[2]);
//             z = z.toString(2).padStart(4, "0");
//             setMachineCode((prev) => prev + z + "\n");
//           }
//         }
//       } catch (error) {
...
//         case arithmeticInstructions.ROL:
//           a[operand1] =
//             (a[operand1] << a[operand2]) | (a[operand1] >> (4 - a[operand2]));
//           break;
//         case arithmeticInstructions.ROR:
//           a[operand1] =
//             (a[operand1] >> a[operand2]) | (a[operand1] << (4 - a[operand2]));
...
//               0
//               // Math.abs(temp) !==
//               //   Math.abs(parseInt(temp.toString(2).slice(-4), 2))
//               //   ? 1
//               //   : 0
//             );
//             //setSf(temp.length > 3 ? a[operand1][-3] : 0);//but i've done arithmatic operation through decimal arithmatic; so, I get 3-7 =-0100, but should've been 1100 in bin operation
...
//         setOf(
//           Math.abs(a[operand1]) !==
//             Math.abs(parseInt(a[operand1].toString(2).slice(-4), 2))
//             ? 1
//             : 0
//         );
//         setSf(a[operand1].length > 3 ? a[operand1][-3] : 0); //but i've done arithmatic operation through decimal arithmatic; so, I get 3-7 =-0100, but should've been 1100 in bin operation
...
//         setZf(a[operand1] === 0 ? 1 : 0);
//       }
//       a[operand1] = parseInt(a[operand1].toString(2).slice(-4), 2);
//       a[operand1] = Math.abs(a[operand1]);
//     } else if (insts[i][0] === "01") {
//       // Arithmetic instruction
//       switch (opcode) {
...
//         case arithmeticInstructions.ROL:
//           a[operand1] =
//             (a[operand1] << operand2) | (a[operand1] >> (4 - operand2));
//           break;
//         case arithmeticInstructions.ROR:
//           a[operand1] =
//             (a[operand1] >> operand2) | (a[operand1] << (4 - operand2));
...
//   return (
//     <div className="bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90% h-[100vh] w-[100vw]">
//       <p className="italic p-2 m-auto font-serif w-[50%]">
...
//                 return (
//                   <p key={index} className={index === i ? "bg-indigo-200" : ""}>
//                     &nbsp;{line && index.toString(2).padStart(7, "0") + ":"}
//                     &nbsp;&nbsp;&nbsp;
//                     {line}
//                   </p>
//                 );
